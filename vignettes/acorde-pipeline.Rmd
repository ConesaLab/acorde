...
![Percentile correlation strategy](../images/percentile_correlations-small.png)
...
We'll use some of the **cluster visualization** functions in acorde to 
see *cluster 6* in `clusters` (`clusters[[7]]`, given that the first element
of the list corresponds to unclustered isoforms) as an example of this:
...
Arguably, some of these isoforms have an expression pattern that is not exactly
equal to the rest of the members of the cluster, for instance, the interval of
scaled expression for the Astroctye and Glutamatergic Neuron cell types is
quite broad. To solve this, we cam use the `filter_clusters()` function, which
will move isoforms to the unclustered groups if they are poorly correlated with 
most of the isoforms in the cluster.
...
initial set of clusters. *cluster 6* now looks like this:

```{r}
# calculate cell type mean expression for all isoforms
example_means.filt <- calculate_cluster_ctmeans(tasic_scaled,
                                           isoform_ids = clusters_filt[[7]], 
                                           id_table = id.df)
...
clusters.expanded <- expand_clusters(tasic_sp, isoform_col = "transcript_id", 
                                     id_table = id.df,
...
# show output format
names(clusters.expanded)
map_chr(clusters.expanded, class)
map_int(clusters.expanded, length)

# check number of unclustered isoforms after first round
length(clusters.expanded$unclustered)


# second round, soft threshold for the rest of the unclustered isoforms
clusters.expanded <- expand_clusters(tasic_sp, isoform_col = "transcript_id",
                                     id_table = id.df,
                                     cluster_list = clusters.expanded$expanded,
                                     unclustered = clusters.expanded$unclustered,
                                     force_expand = FALSE, 
...
As shown above, unclustered isoforms are now assigned to the `unclustered` 
list element, while the list containing the clusters is situated in the `expanded`
slot. The correlation threshold used to assign an isoform to a cluster can be adjusted 
...
map(clusters_merged[1:6], head)
```

We can now check the results of any of the cluster merge decisions made by `merge_clusters` 
(which, after specifying `dynamic = TRUE` runs `cutreeHybrid` under the hood)
...
merge_check <- clusters.expanded$expanded[example_group] %>% 
...
## 8. Optional: manual curation of redundant and/or noisy clusters

As shown above, dynamic clustering of metatranscripts generally works well for 
removing redundancies. However, there are some situations where fully-automated 
merge does not perform as expected, and we may want more control over the results. 
In particular, we will next focus on manual curation of **redundancy**, i.e. 
...
resulted in joining clusters with clearly uncorrelated profiles.
